...
public class EntityConverter {
  ...
  public OfferDto OfferToDto(Offer offer) {
    return new OfferDto(
      ...
      LocationToDto(offer.getLocation()),
      CategoryToDto(offer.getCategory()),
      UserToSimpleDto(offer.getUser()),
      ResponsesToDto(offer.getResponses())
    );
  }
  ...
  public List<ResponseDto> ResponsesToDto(List<Response> responses) {
    return responses
      .stream()
      .map(response ->
        new ResponseDto(response.getId(), response.getReply(), response.getCv())
      )
